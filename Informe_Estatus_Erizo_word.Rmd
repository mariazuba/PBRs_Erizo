...
ppredF <-rep1$ppred[37:60,]
anos   <-yrs[37:60]
obsF   <-rep1$pobs[37:60,]
preF   <-ppredF
...
```{r Fig_RetrospectivoXN, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Patrón retrospectivo estándar (panel izquierdo) y  relativo (panel derecho) de los reclutamientos", fig.height=7, fig.width=6}
...
```{r Fig_DiagramaFaseXN, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**. Diagrama de fase Erizo zona X Norte", fig.height=6, fig.width=8}
...
x.bottom.right<- c(0, 1, 4, 4)
x.top.right   <- c(0, 1, 4, 4)
...
# plot
ggplot() + 
  geom_polygon(data = data.df, aes(x = x, y = y, fill = factor(value), group = id)) +
  geom_line(aes(x=bd_bdrms,y=f_frms))+geom_point(aes(x=bd_bdrms,y=f_frms),shape = 21, colour = "black", fill = "white", size = 3, stroke = 1)+
scale_fill_grey(start=0.4, end=0.9) +
geom_hline(yintercept = 1) + geom_vline(xintercept = c(0.5,1),colour=c(2,1)) +
annotate("text", x=c(0.5,1,4.05), y=c(10.05,10.05,1.05),
              label=c(expression("BD"[LIM]),expression("BD"[RMS]),expression("F"[RMS]))) +

labs(x = 'BD/BDrms', y = 'F/Frms',fill='Estados de explotación') +
...
```{r Fig_DiagramaFaseXS, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**. Diagrama de fase Erizo zona X Sur", fig.height=6, fig.width=8}
...
```{r Fig_DiagramaFaseXI, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**. Diagrama de fase Erizo zona XI", fig.height=6, fig.width=8}
Zona<- "Erizo Zona XI"
Frms<-dat3$FMRS
BDo<-rep3$BDo
BDrms<-BDo*0.4
