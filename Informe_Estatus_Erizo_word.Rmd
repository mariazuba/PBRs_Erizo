...
nyearc1  <- length(yearc1)  
 ...
 pred_sep <- as.data.frame(etcf1_pre) %>% mutate(year=yearc1) %>% melt(id.vars='year') %>%
  ...
  fig1 <- ggplot(filter(mat, type=='obs')) + 
          ...
          facet_wrap(~year, dir = 'v', as.table = TRUE) + labs(x = 'Longitud', y = 'Proporción') +
...
for(i in 1:nretros){
  rep<- reptoRlist(paste(admb,"s",i,".rep",sep=""))
  retroR[,i] <- c(rep$R_Est)
  retroBD[,i] <- c(rep$BD)
  retroF[,i]  <- c(rep$F) }
  colnames(retroR)<-paste("y",seq(1,nretros,1),sep="")
  colnames(retroBD)<-paste("y",seq(1,nretros,1),sep="")
  colnames(retroF)<-paste("y",seq(1,nretros,1),sep="")

dataR <- as.data.frame(retroR) %>% mutate(year=years) %>% melt(id.vars='year')
...
```{r Fig_MortalidadNatural_XN, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad de la Mortalidad natural de erizo de la zona norte", fig.height=5, fig.width=5}


Rt <- ggplot() + 
      geom_ribbon(data=Rt_retro, aes(ymin=lower, ymax=upper, x=x, fill = "Intervalo de confianza asintótico"), alpha = 0.2)+
      geom_line(data=dataR, aes(y=value, x=year, colour = variable), size=0.5)+
      labs(x = '', y = 'Reclutamientos',colour='Mortalidad natural')  +
      scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 10)) +
      theme_bw(base_size=8) +
      ggtitle('')+
      theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
BD <- ggplot() + 
      geom_ribbon(data=BD_retro,aes(ymin=lower, ymax=upper, x=x, fill = ""), alpha = 0.2)+
...
```{r Fig_Loo_XN, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad del rango de Loo de erizo de la zona norte", fig.height=5, fig.width=5}


Rt <- ggplot() + 
      geom_ribbon(data=Rt_retro, aes(ymin=lower, ymax=upper, x=x, fill = "Intervalo de confianza asintótico"), alpha = 0.2)+
      geom_line(data=dataR, aes(y=value, x=year, colour = variable), size=0.5)+
      labs(x = '', y = 'Reclutamientos',colour='Loo')  +
      scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 10)) +
      theme_bw(base_size=8) +
      ggtitle('')+
      theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
```{r Fig_MortalidadNatural_XS, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad de la Mortalidad natural de erizo de la zona X sur", fig.height=5, fig.width=5}
...
Ft <- ggplot() + 
      geom_line(data=dataF, aes(y=value, x=year, colour = variable), size=0.5)+
      geom_ribbon(data=Ft_retro,aes(ymin=lower, ymax=upper, x=x, fill = ""), alpha = 0.2)+
      labs(x = '', y = 'Mortalidad por pesca',colour='Mortalidad natural')  +
      scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 10)) +
      theme_bw(base_size=8) +
      ggtitle('')+
...
```{r Fig_Loo_XS, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad del rango de Loo de erizo de la zona X sur", fig.height=7, fig.width=5}
...
```{r Fig_MortalidadNatural_XI, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad de la Mortalidad natural de erizo de la zona XI", fig.height=5, fig.width=5}
...
```{r Fig_Loo_XI, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="**Figura x**.	Análisis de sensibilidad del rango de Loo de erizo de la zona XI", fig.height=7, fig.width=5}
